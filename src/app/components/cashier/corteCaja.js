...
export default function CorteDeCaja() {
	...
	const print = async (order) => {
		const hasConcept =
			...
			order.concept !== null &&
			order.concept !== "" &&
			order.concept !== "undefined";

		const descuento =
			order.discount !== "0" && order.discount !== "" && order.discount != null
				? order.discount
				: 0;
    ...
    <html>
      ...
      <body style="text-align: center; justify-content:space-around;">
        ...
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tbody>
            ${order?.pretty_list
                ...
                ${Object.keys(product?.supplies_modified)
									.map((key) => {
        ...
        ${
          ...
          <br>
          <br>
          Propina sugerida (10%): ${priceFormatter.format(total * 0.1)}<br>
          Propina sugerida (15%): ${priceFormatter.format(total * 0.15)}<br>
          Propina sugerida (20%): ${priceFormatter.format(total * 0.2)}<br>
          <br>
          <br>
          <br>
	...
	const inventoryReport = () => {
		...
		const tables = orders?.data?.map(
			(order) =>
				new ReportTable({
					header: [
						"PRODUCTO",
						`PRECIO (${order?.is_effective ? "E" : "T"})`,
						`MESA ${order?.table?.name}`,
						`MESERO ${order?.waiter_name}`.toUpperCase(),
					],
  ...
  ${CSSPDF}
	...
	const inventoryReportExcel = () => {
		...
		const matriz = orders?.data?.reduce((acc, curr, index) => {
			...
			const header = [
				`FOLIO ${curr?.folio}`,
				`MESERO ${curr?.waiter_name}`.toUpperCase(),
				`MESA ${curr?.table?.name}`,
				curr?.is_effective ? "EFECTIVO" : "TARJETA",
			];
		...
		const totalDebit = [
			"TOTAL EN TARJETA",
			priceFormatter.format(orders?.total_debit),
		];
		const totalCash = [
			"TOTAL EN EFECTIVO",
			priceFormatter.format(orders?.total_cash),
		];
		...
		workbook.xlsx
			...
			.then((buffer) => {
				FileSystem.writeAsStringAsync(filePath, buffer.toString("base64"), {
	...
	return (
		<View style={{ flex: 1 }}>
			...
			>
				...
				>
					<View
						style={{ backgroundColor: "white", padding: 20, borderRadius: 7 }}
					>
			...
			<View style={{ flex: 1, paddingVertical: 40 }}>
						...
						return (
							...
							>
								...
								<Text style={{ ...styles.text, textAlign: "right" }}>
									{" "}
									Total:{priceFormatter.format(item?.total)}{" "}
								</Text>
							</TouchableOpacity>
						);
					})}
				...
				<Text style={styles.text}>
					Total:{priceFormatter.format(orders?.total)}
				</Text>
				<View
					style={{
						flexDirection: "row",
