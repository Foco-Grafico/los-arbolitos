...
export default function Cashier() {
	...
	const print = async (isEffective) => {
		...
		const hasConcept =
			concept !== "" && concept !== null && concept !== undefined;

		const totalWithDiscount = Number(
			selectedTable?.total + Number(extraPrice) - discount,
		);
		...
		try {
			...
			setData((prev) => {
				...
				const index = copyPrev.findIndex(
					(order) => order.id === selectedTable?.id,
				);
	...
	return (
		<View style={styles.container}>
			...
			<View style={styles.header}>
				<TouchableOpacity onPress={reloadOrders}>
					<IconRefresh fill="#fff" style={{ width: 20, height: 20 }} />
				</TouchableOpacity>
				<Text
					style={{
						color: "white",
